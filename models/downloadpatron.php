...
class Downloadpatron extends AppModel {
    ...
    function getDaysDownloadInformation($libraryID, $date, $territory) {
        ...
        $record = $this->find('all', array('conditions' => $conditions, 'fields' => array('Currentpatrons.id, Currentpatrons.branch_name, `Downloadpatron`.`download_date`, `Downloadpatron`.`library_id`, `Downloadpatron`.`patron_id`, `Downloadpatron`.`email`, `Downloadpatron`.`total`,`Library`.`show_barcode`'),
