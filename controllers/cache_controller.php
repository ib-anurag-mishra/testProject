...
STR;
    ...
    try {
      ...
      $this->Song->bindModel(array(
                  'hasOne' => array(
                    'Country' => array(
                      'foreignKey' => false,
                      'type' => 'INNER',
                      'conditions' => array('Country.ProdID = Song.ProdID AND Country.provider_type = Song.provider_type')
                    ), 
                  )
                ));
      ...
      $gcondition = array('Song.DownloadStatus_nayan1' => 1, "Song.Sample_FileID != ''", "Song.FullLength_FIleID != ''", "Song.ArtistText != ''", $condition, '1 = 1 GROUP BY Song.ArtistText');
       $this->paginate = array(
                    'conditions' => $gcondition,
                    'fields' => array('DISTINCT Song.ArtistText_more1'),
                    'extra' => array('chk' => 1),
                    ...
                    'check' => 2,
                    'all_query' => true,
                    'all_country' => $countryPrefix.'countries_azine1',
      ...
      $allArtists = $this->paginate('Song'); exit;
      for($j = 65;$j < 93;$j++){
        $alphabet = chr($j);
        if($alphabet == '[') {
        ...
        $this->Song->Behaviors->attach('Containable');
        $this->Song->recursive = 0;
        $gcondition =array('Song.DownloadStatus' => 1, "Song.Sample_FileID != ''", "Song.FullLength_FIleID != ''", "Song.ArtistText != ''", $condition, '1 = 1 GROUP BY Song.ArtistText');
        $this->paginate = array(
                        'conditions' => $gcondition,
                        ...
                        'check' => 2,
                        'all_query' => true,
                        'all_country' => $countryPrefix.'countries',
                        'all_condition' => ((is_array($condition) && isset($condition['Song.ArtistText LIKE'])) ? "Song.ArtistText LIKE '" . $condition['Song.ArtistText LIKE'] . "'" : (is_array($condition) ? $condition[0] : $condition))
                    );
      ...
      foreach($genreAll as $genreRow){
        ...
        $condition = "";
        $this->Song->unbindModel(array('hasOne' => array('Participant')));
        $this->Song->hasOne['Country']['type'] = 'INNER';
        $this->Song->unbindModel(array('belongsTo' => array('Sample_Files','Full_Files')));
        $this->Song->Behaviors->attach('Containable');
        $this->Song->recursive = 0;
       $this->paginate = array(
                      'conditions' => array(
                        ...
                        'Country.Territory IS NOT NULL',
                        "Song.Sample_FileID != ''", 
                        "Song.FullLength_FIleID != ''", 
                        $condition, 
                        ' 1 = 1 GROUP BY Song.ArtistText'
                      ),
                      'fields' => array(
                      ...
                      'contain' => array(
                        ...
                        'Country' => array(
                          ...
                          'conditions' => array(
                            'Country.ProdID = Song.ProdID AND Country.provider_type = Song.provider_type',               
                          ),         
                        ),
                      ),
                      'extra' => array('chk' => 1),
