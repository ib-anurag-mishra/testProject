...
Class ArtistsController extends AppController {
    ...
    function admin_managetopsingles($territory = null) {
	$userTypeId = $this->Session->read('Auth.User.type_id');
        ...
        $topSingles = $this->TopSingles->getAdminTopSingles($territory);
    ...
    function admin_getterritorytopsingles($territory) {
        $this->layout = 'ajax';
	$userTypeId = $this->Session->read('Auth.User.type_id');
        $topSingles = $this->TopSingles->getAdminTopSingles($territory);
	$this->set('userTypeId',$userTypeId);
    ...
    function admin_saveTopsinglesSortOrder($territory) {
	...
	$insertArr['prod_id'] = $songID;
        ...
        $query = "SELECT IFNULL(MAX(sortId),0)+1 AS sortId FROM top_singles";
    ...
    function admin_managetopalbums($territory = null) {
	$userTypeId = $this->Session->read('Auth.User.type_id');
        $territories = $this->Territory->find("all");
        for ($m = 0; $m < count($territories); $m++) {
            $territoriesArray[$territories[$m]['Territory']['Territory']] = $territories[$m]['Territory']['Territory'];
    ...
    function view($id = null, $album = null, $provider = null) {
        ...
        if (!empty($albumSongs)) {
            ...
            //if no any song available to display or
            //if all songs in a album has sales date greater then today's date
            if(empty($albumSongs[$albumReferenceID])) { 
                 $this->set('blockHoleContent', 1);      
            }else{
                $this->set('blockHoleContent', 0);  
            }
        }        
        ...
        if ($album != '') {
            ...
            $songs = $this->Song->find('all', array(
                ...
                'conditions' => array('Song.ArtistText' => base64_decode($id), "Song.Sample_FileID != ''", "Song.FullLength_FIleID != ''", 'Country.Territory' => $country,
                    "Country.SalesDate <= NOW()","Country.StreamingSalesDate <= NOW()",'Country.DownloadStatus' => 1,
                    array('or' =>
                        array(
                            array('Country.StreamingStatus' => 1)
    ...
    function album($id = null, $album = null, $provider = null) {
        ...
        if (!empty($country)) {
            //for time being we have added this after when we switch cache this code will be remove
