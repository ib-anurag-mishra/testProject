...
Class ArtistsController extends AppController {
	...
	*/
	...
	function view($id = null, $album = null, $provider = null) {
		...
		//this flage will be use when $providerType value found empty
		$checkProviderTypeFlage = 0;

		//check if album value is set in url
		if ( $album != '' ) {
			$providerType = "(" . $album . ",'" . $provider . "')";
			...
			if ( isset( $songs ) && is_array( $songs ) && count( $songs ) > 0 ) {
				foreach ($songs as $k => $v) {
					...
					$prodId 	  .= $v['Song']['ReferenceID'] . ",";
					$providerType .= "(" . $v['Song']['ReferenceID'] . ",'" . $v['Song']['provider_type'] . "'),";
				}

				$prodId 	  = rtrim( $prodId, ',' );
				$providerType = rtrim( $providerType, ',' );
			}
		...
		if( $checkProviderTypeFlage == 0 ) {
			$albumData = $this->Album->fetchAlbumData( $prodId, $providerType, 'view' ); //getting the Albums for the artist
		}

		if ( isset( $albumData ) && is_array( $albumData ) && count( $albumData ) > 0 && $libType == 2 ) {
		...
		if ( isset( $albumSongs ) && is_array($albumSongs) && count( $albumSongs ) > 0 ) {
	...
	*/
	...
	public function album( $id = null, $album = null, $provider = null ) {

		$country 	 = $this->Session->read( 'territory' );
		$patronId	 = $this->Session->read( 'patron' );
		$libraryId	 = $this->Session->read( 'library' );
		$libraryType = $this->Session->read( 'library_type' );
		...
		$songs = $this->Song->getArtistAlbums( $id, $country, $cond );
		...
		if ( is_array( $songs ) && count( $songs ) > 0 ) {
			...
			foreach ($songs as $value) {
				$prodId 	  .= $value['Song']['ReferenceID'] . ',';
				$providerType .= "(" . $value['Song']['ReferenceID'] . ",'" . $value['Song']['provider_type'] . "')" . ',';
			}

			$prodId 	  = rtrim($prodId, ',');
			$providerType = rtrim($providerType, ',');

			$albumData = $this->Album->fetchAlbumData( $prodId, $providerType );
