...
class SoapsController extends AppController {
    ...
    function getLibrary($data) {
        ...
        if (is_numeric($data)) {
            ...
            $libraries = $this->Library->find('all', array('fields' =>
                array(
                    'Library.id',
                    'library_name',
                    'library_apikey',
    ...
    function getAlbumsFromArtistText($authenticationToken, $artistText, $startFrom, $recordCount) {
        ...
        for ($cnt = $startFrom; $cnt < ($startFrom + $recordCount); $cnt++) {
            if (!(empty($albumData[$cnt]))) {
                ...
                $fileURL = $this->Token->regularToken($albumData[$cnt]['Album']['CdnPath'] . "/" . $albumData[$cnt]['Album']['Image_SaveAsName']);
                $fileURL = Configure::read('App.Music_Path') . $fileURL;
                $obj->FileURL = $fileURL;

                if ('T' == $albumData[$cnt]['Album']['Advisory']) {
                    $obj->AlbumTitle = $obj->AlbumTitle . ' (Explicit)';
                    $obj->Title = $obj->Title . ' (Explicit)';
                }
        ...
        if (!(empty($nationalTopDownload))) {
            ...
            foreach ($nationalTopDownload as $key => $data) {
                ...
                if ($this->IsDownloadable($data['Song']['ProdID'], $territory, $data['Song']['provider_type'])) {
                    ...
                    $obj->FullLength_FIleURL = Configure::read('App.Music_Path') . $this->Token->regularToken($data['Song']['CdnPath'] . "/" . $data['Song']['FullLength_SaveAsName']);
                }
                ...
                if ('T' == $data['Song']['Advisory'])
                    $obj->SongTitle = $obj->SongTitle . ' (Explicit)';
    ...
    function getAlbumDetail($authenticationToken, $prodId) {
        ...
        if (!empty($albumData)) {
            ...
            if (empty($obj->AlbumTitle)) {
                $obj->AlbumTitle = ' ';
            }
            if (empty($obj->Title)) {
                $obj->Title = ' ';
            }
            if (empty($obj->ArtistText)) {
                $obj->ArtistText = ' ';
            }
            if (empty($obj->Artist)) {
                $obj->Artist = ' ';
            }
            if (empty($obj->ArtistURL)) {
                $obj->ArtistURL = ' ';
            }
            if (empty($obj->Label)) {
                $obj->Label = ' ';
            }
            if (empty($obj->Copyright)) {
                $obj->Copyright = ' ';
            }
            if (empty($obj->Advisory)) {
                $obj->Advisory = ' ';
            }
            if (empty($obj->FileURL)) {
                $obj->FileURL = ' ';
            }
            if (empty($obj->DownloadStatus)) {
                $obj->DownloadStatus = ' ';
            }
            if (empty($obj->TrackBundleCount)) {
                $obj->TrackBundleCount = ' ';
            }

            if ('T' == $data['Album']['Advisory']) {
                $obj->AlbumTitle = $obj->AlbumTitle . ' (Explicit)';
                $obj->Title = $obj->Title . ' (Explicit)';
            }
            ...
            foreach ($data['Song'] AS $val) {
                ...
                if ($this->IsTerrotiry($val['ProdID'], $val['provider_type'], $libraryId)) {
                    ...
                    if (empty($sobj->SongTitle)) {
                        $sobj->SongTitle = ' ';
                    }
                    if (empty($sobj->ArtistText)) {
                        $sobj->ArtistText = ' ';
                    }
                    if (empty($sobj->Artist)) {
                        $sobj->Artist = ' ';
                    }
                    if (empty($sobj->Advisory)) {
                        $sobj->Advisory = ' ';
                    }
                    if (empty($sobj->ISRC)) {
                        $sobj->ISRC = ' ';
                    }
                    if (empty($sobj->Composer)) {
                        $sobj->Composer = ' ';
                    }
                    if (empty($sobj->Genre)) {
                        $sobj->Genre = ' ';
                    }
                    if (empty($sobj->Territory)) {
                        $sobj->Territory = ' ';
                    }
                    if (empty($sobj->DownloadStatus)) {
                        $sobj->DownloadStatus = ' ';
                    }
                    if (empty($sobj->TrackBundleCount)) {
                        $sobj->TrackBundleCount = ' ';
                    }
                    if (empty($sobj->Sample_Duration)) {
                        $sobj->Sample_Duration = ' ';
                    }
                    if (empty($sobj->FullLength_Duration)) {
                        $sobj->FullLength_Duration = ' ';
                    }
                    if (empty($sobj->Sample_FileID)) {
                        $sobj->Sample_FileID = ' ';
                    }
                    if (empty($sobj->Sample_FileURL)) {
                        $sobj->Sample_FileURL = ' ';
                    }
                    if (empty($sobj->FullLength_FIleURL)) {
                        $sobj->FullLength_FIleURL = ' ';
                    }
                    if (empty($sobj->FullLength_FIleID)) {
                        $sobj->FullLength_FIleID = ' ';
                    }
                    if (empty($sobj->CreatedOn)) {
                        $sobj->CreatedOn = ' ';
                    }
                    if (empty($sobj->UpdateOn)) {
                        $sobj->UpdateOn = ' ';
                    }
    ...
    function addSongToWishlist($libraryId, $prodId, $authenticationToken, $isrc, $userAgent) {
        ...
        $cnt = $this->Wishlist->find('count', array('conditions' =>
        ...
        if ($wishlistCount >= $patronLimit && $libraryDownload != '1' && $patronDownload != '0') {
            ...
            $TrackData = $this->Song->find('first', array(
                'fields' => array(
                    'Song.ProdID',
                    'Song.ProductID',
                    'Song.Title',
    ...
    function getWishlist($authenticationToken) {
        ...
        $wishlistResults = $this->Wishlist->find('all', array(
            'fields' => array(
                'Wishlist.id',
                'Wishlist.library_id',
                'Wishlist.patron_id',
    ...
    function regenerateSongDownloadRequest($authentication_token, $prodId, $agent) {
        ...
        $data = $this->Song->find('first', array('joins' =>
            array(
                array(
                    'table' => 'File',
                    'alias' => 'f',
    ...
    function regenerateVideoDownloadRequest($authentication_token, $prodId, $agent) {
        ...
        $data = $this->Video->find('first', array('joins' =>
            array(
                array(
                    'table' => 'File',
                    'alias' => 'f',
    ...
    function songDownloadRequest($authentication_token, $prodId, $agent) {
        ...
        if (is_numeric($return)) {
            ...
            $data = $this->Files->find('first', array(
                'fields' => array(
                    'CdnPath',
                    'SaveAsName'
                ),
    ...
    function videoDownloadRequest($authentication_token, $prodId, $agent) {
        ...
        $TrackData = $this->Video->find('first', array(
            'fields' => array(
                'Video.ProdID',
                'Video.ProductID',
                'Video.Title',
        ...
        if ((($topSinglesCache) !== false) && ($topSinglesCache !== null)) {
            ...
            for ($cnt = $startFrom; $cnt < ($startFrom + $recordCount); $cnt++) {
                if (!(empty($arrTemp[$cnt]))) {
                    ...
                    if ('T' == $arrTemp[$cnt]['Song']['Advisory'])
                        $sobj->SongTitle = $sobj->SongTitle . ' (Explicit)';
    ...
    function getMyMusicVideos($authenticationToken, $startFrom, $recordCount) {
        ...
        v.FullLength_Duration, v.DownloadStatus, v.Advisory, c.SalesDate, gr.Genre, v.FullLength_SaveAsName, v.Image_SaveAsName, v.CdnPath, prd.pid, COUNT(vd.id) AS cnt
        FROM video AS v
        ...
        for ($cnt = $startFrom; $cnt < ($startFrom + $recordCount); $cnt++) {
            if (!(empty($arrTemp[$cnt]))) {
                ...
                $sobj->VideoImage_FileURL = Configure::read('App.Music_Path') . $this->Token->artworkToken($arrTemp[$cnt]['v']['CdnPath'] . "/" . $arrTemp[$cnt]['v']['Image_SaveAsName']);
    ...
    function getGenreSongs($authenticationToken, $genreTitle) {
        ...
        if ((($genreSongsCache ) !== false) && ($genreSongsCache !== null)) {
            ...
            foreach ($genreSongsCache AS $key => $val) {
                ...
                $sampleFileURL = $this->Token->regularToken($val['Song']['CdnPath'] . "/" . $val['Song']['Sample_SaveAsName']);
                ...
                if ($sobj->DownloadStatus) {
                    ...
                    $sobj->Sample_FileURL = Configure::read('App.Music_Path') . $sampleFileURL;
                    $sobj->FullLength_FIleURL = Configure::read('App.Music_Path') . $this->Token->regularToken($val['Song']['CdnPath'] . "/" . $val['Song']['FullLength_SaveAsName']);
                }
    ...
    private function getSearchAlbumList($searchText, $startFrom, $recordCount, $searchType, $libraryId, $library_terriotry) {
        ...
        foreach ($Albumlist AS $key => $val) {
            ...
            if ('T' == $albumData['Album']['Advisory']) {
                $sobj->AlbumTitle = $sobj->AlbumTitle . ' (Explicit)';
            }
    ...
    private function getSearchVideoSongList($searchText, $startFrom, $recordCount, $searchType, $libraryId, $library_terriotry) {
        ...
        foreach ($VideoData AS $key => $val) {
            ...
            if ($sobj->DownloadStatus) {
                ...
                $vdata = $this->Files->find('first', array(
                    'fields' => array(
                        'CdnPath',
                        'SaveAsName'
                    ),
    ...
    private function getDownloadStatusOfSong($ProdID, $provider_type, $library_territory) {
        ...
        $DownloadStatus = $this->Country->find('first', array(
            'fields' => array(
    ...
    private function IsTerrotiry($songProdID, $provider_type, $libraryId) {
        ...
        $count = $this->Country->find('count', array(
