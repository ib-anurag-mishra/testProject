...
Class DownloadsComponent extends Object
{
    ...
    {
        /**
         * Download count to be check from sesison first
         * if not set then query is executed for songs and videos download
         * else session value is fetched for download count
         */
        if (!$this->Session->check('downloadCount'))
        {
            $downloadInstance = ClassRegistry::init('Download');
            $downloadInstance->recursive = -1;
            $results = $downloadInstance->find(
                    'count', array(
                'conditions' => array(
                    'library_id' => $libId,
                    'patron_id' => $patId,
                    'created BETWEEN "' . Configure::read('App.curWeekStartDate') . '" and "' . Configure::read('App.curWeekEndDate') . '" ')
                    )
            );
        ...
        {
             $downloadCount = $this->Session->read('downloadCount');
        }

        
        //patrom limit count query
        $libraryInstance = ClassRegistry::init('Library');
        $libraryInstance->recursive = -1;
