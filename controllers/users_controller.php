...
Class UsersController extends AppController
{
	...
	var $uses = array('User','Group', 'Library', 'Currentpatron', 'Download','Variable','Url','Language','Consortium','Card','LibrariesTimezone','NotificationSubscriptions','MultiAuthentication');
...
function login($library = null){
			...
			if($authMethod == 'user_account'){
				...
				// do nothing
					...
					$this->Currentpatron->save($insertArr);
                                        $this->log("users/index: [libid=".$libraryId.", patronid=".$patronId.", session_id=".$insertArr['session_id'].", CNT=".count($currentPatron).", last inserted ID=".$this->Currentpatron->getLastInsertId()."]", "currentpatrons");
				}
				$date = time();
        ...
        function savestreampopup(){
            ...
            if(isset($this->params[$index]['pid']) && isset($this->params[$index]['lid']) && $this->params[$index]['lid']!=''  && $this->params[$index]['pid']!='') 
            {
                ...
                if(count($currentPatronData) > 0) {
                        ...
                        $this->log("users/savestreampopup: [libid=".$libaryID.", patronid=".$patronId.", CNT=".count($currentPatronData).", last inserted ID=".$this->Currentpatron->getLastInsertId()."]", "currentpatrons");
        ...
        function savenotifypopup(){
            ...
            if(isset($_REQUEST['pid']) && isset($_REQUEST['lid']) && $_REQUEST['lid']!=''  && $_REQUEST['pid']!='') 
            {
                ...
                if(count($currentPatronData) > 0) {
                        ...
                        $this->Currentpatron->set($currentPatronData['Currentpatron']);
                        $this->Currentpatron->save();
                        $this->log("users/savenotifypopup: [libid=".$libaryID.", patronid=".$patronId.", CNT=".count($currentPatronData).", last inserted ID=".$this->Currentpatron->getLastInsertId()."]", "currentpatrons");
                        $this->Session->write('showNotificationPopup','yes');
                }
                $this->Currentpatron->setDataSource('default');         
            }
				...
				if(count($existingLibraries) == 0){
					...
					if($resultAnalysis[0] == "fail"){
						...
						// do nothing
							...
							$insertArr['session_id'] = session_id();
							$this->Currentpatron->save($insertArr);
                                                        $this->log("users/ilogin: [libid=".$existingLibraries['0']['Library']['id'].", patronid=".$patronId.", session_id=".$insertArr['session_id'].", CNT=".count($currentPatron).", last inserted ID=".$this->Currentpatron->getLastInsertId()."]", "currentpatrons");
						}
						$date = time();
						$values = array(0 => $date, 1 => session_id());
				   ...
				   $this->redirect(array('controller' => 'users', 'action' => 'idlogin'));
					...
					if($resultAnalysis[0] == "fail"){
						...
						// do nothing
							...
							$insertArr['session_id'] = session_id();
							$this->Currentpatron->save($insertArr);
                                                        $this->log("users/idlogin: [libid=".$existingLibraries['0']['Library']['id'].", patronid=".$patronId.", session_id=".$insertArr['session_id'].", CNT=".count($currentPatron).", last inserted ID=".$this->Currentpatron->getLastInsertId()."]", "currentpatrons");
						}
						$date = time();
						$values = array(0 => $date, 1 => session_id());
						Cache::write("login_".$existingLibraries['0']['Library']['library_territory']."_".$existingLibraries['0']['Library']['id']."_".$patronId, $values);
				   ...
				   $this->redirect(array('controller' => 'users', 'action' => 'mdlogin'));
					...
					if(isset($login_res['Card']['id'])) {
						...
						// do nothing
							...
							$insertArr['session_id'] = session_id();
							$this->Currentpatron->save($insertArr);
                                                        $this->log("users/mdlogin: [libid=".$existingLibraries['0']['Library']['id'].", patronid=".$patronId.", session_id=".$insertArr['session_id'].", CNT=".count($currentPatron).", last inserted ID=".$this->Currentpatron->getLastInsertId()."]", "currentpatrons");
						}
						$date = time();
						$values = array(0 => $date, 1 => session_id());
						Cache::write("login_".$existingLibraries['0']['Library']['library_territory']."_".$existingLibraries['0']['Library']['id']."_".$patronId, $values);
				...
				$login_res = $this->Card->find('first',array('conditions' => array('Card.card_number' => $card , 'Card.library_id' => $existingLibraries['0']['Library']['id'] ) , 'fields' => array('id')));
					if(isset($login_res['Card']['id'])) {
						...
						// do nothing
							...
							$insertArr['session_id'] = session_id();
							$this->Currentpatron->save($insertArr);
                                                        $this->log("users/mndlogin: [libid=".$existingLibraries['0']['Library']['id'].", patronid=".$patronId.", session_id=".$insertArr['session_id'].", CNT=".count($currentPatron).", last inserted ID=".$this->Currentpatron->getLastInsertId()."]", "currentpatrons");
						}
						$date = time();
						$values = array(0 => $date, 1 => session_id());
						Cache::write("login_".$existingLibraries['0']['Library']['library_territory']."_".$existingLibraries['0']['Library']['id']."_".$patronId, $values);
				...
				if(count($existingLibraries) == 0){
				   ...
				   $this->redirect(array('controller' => 'users', 'action' => 'ildlogin'));
					...
					// do nothing
						...
						$insertArr['session_id'] = session_id();
						$this->Currentpatron->save($insertArr);
                                                $this->log("users/ildlogin: [libid=".$existingLibraries['0']['Library']['id'].", patronid=".$patronId.", session_id=".$insertArr['session_id'].", CNT=".count($currentPatron).", last inserted ID=".$this->Currentpatron->getLastInsertId()."]", "currentpatrons");
					}
					$date = time();
					$values = array(0 => $date, 1 => session_id());
					Cache::write("login_".$existingLibraries['0']['Library']['library_territory']."_".$existingLibraries['0']['Library']['id']."_".$patronId, $values);	
				...
				if(count($existingLibraries) == 0){
					...
					if($resultAnalysis[0] == "fail"){
						...
						// do nothing
							...
							$insertArr['session_id'] = session_id();
							$this->Currentpatron->save($insertArr);
                                                        $this->log("users/inlogin: [libid=".$existingLibraries['0']['Library']['id'].", patronid=".$patronId.", session_id=".$insertArr['session_id'].", CNT=".count($currentPatron).", last inserted ID=".$this->Currentpatron->getLastInsertId()."]", "currentpatrons");
						}
						$date = time();
						$values = array(0 => $date, 1 => session_id());
						Cache::write("login_".$existingLibraries['0']['Library']['library_territory']."_".$existingLibraries['0']['Library']['id']."_".$patronId, $values);
	...
	function sso(){
		...
		if(count($existingLibraries) == 0){
			...
			// do nothing
				...
				$insertArr['session_id'] = session_id();
				$this->Currentpatron->save($insertArr);
                                $this->log("users/sso: [libid=".$existingLibraries['0']['Library']['id'].", patronid=".$card.", session_id=".$insertArr['session_id'].", CNT=".count($currentPatron).", last inserted ID=".$this->Currentpatron->getLastInsertId()."]", "currentpatrons");
			}
			$date = time();
			$values = array(0 => $date, 1 => session_id());
				...
				if(count($existingLibraries) == 0){
					...
					if($resultAnalysis[0] == "fail"){
						...
						// do nothing
							...
							$insertArr['session_id'] = session_id();
							$this->Currentpatron->save($insertArr);
                                                        $this->log("users/inhlogin: [libid=".$existingLibraries['0']['Library']['id'].", patronid=".$patronId.", session_id=".$insertArr['session_id'].", CNT=".count($currentPatron).", last inserted ID=".$this->Currentpatron->getLastInsertId()."]", "currentpatrons");
						}
						$date = time();
						$values = array(0 => $date, 1 => session_id());
						Cache::write("login_".$existingLibraries['0']['Library']['library_territory']."_".$existingLibraries['0']['Library']['id']."_".$patronId, $values);
				...
				if(count($existingLibraries) == 0){
					...
					if($resultAnalysis[0] == "fail"){
						...
						// do nothing
							...
							$insertArr['session_id'] = session_id();
							$this->Currentpatron->save($insertArr);
                                                        $this->log("users/capita: [libid=".$existingLibraries['0']['Library']['id'].", patronid=".$patronId.", session_id=".$insertArr['session_id'].", CNT=".count($currentPatron).", last inserted ID=".$this->Currentpatron->getLastInsertId()."]", "currentpatrons");
						}
						$date = time();
						$values = array(0 => $date, 1 => session_id());
						Cache::write("login_".$existingLibraries['0']['Library']['library_territory']."_".$existingLibraries['0']['Library']['id']."_".$patronId, $values);
				...
				if(count($existingLibraries) == 0){
					...
					if($resultAnalysis[0] == "fail"){
						...
						// do nothing
							...
							$insertArr['session_id'] = session_id();
							$this->Currentpatron->save($insertArr);
                                                        $this->log("users/symws: [libid=".$existingLibraries['0']['Library']['id'].", patronid=".$patronId.", session_id=".$insertArr['session_id'].", CNT=".count($currentPatron).", last inserted ID=".$this->Currentpatron->getLastInsertId()."]", "currentpatrons");
						}
						$date = time();
						$values = array(0 => $date, 1 => session_id());
						Cache::write("login_".$existingLibraries['0']['Library']['library_territory']."_".$existingLibraries['0']['Library']['id']."_".$patronId, $values);
				   ...
				   $this->redirect(array('controller' => 'users', 'action' => 'ihdlogin'));
					...
					if($resultAnalysis[0] == "fail"){
						...
						// do nothing
							...
							$insertArr['session_id'] = session_id();
							$this->Currentpatron->save($insertArr);
                                                        $this->log("users/ihdlogin: [libid=".$existingLibraries['0']['Library']['id'].", patronid=".$patronId.", session_id=".$insertArr['session_id'].", CNT=".count($currentPatron).", last inserted ID=".$this->Currentpatron->getLastInsertId()."]", "currentpatrons");
						}
						$date = time();
						$values = array(0 => $date, 1 => session_id());
						Cache::write("login_".$existingLibraries['0']['Library']['library_territory']."_".$existingLibraries['0']['Library']['id']."_".$patronId, $values);
				   ...
				   $this->redirect(array('controller' => 'users', 'action' => 'inhdlogin'));
					...
					if($resultAnalysis[0] == "fail"){
						...
						// do nothing
							...
							$insertArr['session_id'] = session_id();
							$this->Currentpatron->save($insertArr);
                                                        $this->log("users/inhdlogin: [libid=".$existingLibraries['0']['Library']['id'].", patronid=".$patronId.", session_id=".$insertArr['session_id'].", CNT=".count($currentPatron).", last inserted ID=".$this->Currentpatron->getLastInsertId()."]", "currentpatrons");
						}
						$date = time();
						$values = array(0 => $date, 1 => session_id());
						Cache::write("login_".$existingLibraries['0']['Library']['library_territory']."_".$existingLibraries['0']['Library']['id']."_".$patronId, $values);
				...
				if(count($existingLibraries) == 0){
					...
					if($resultAnalysis[0] == "fail"){
						...
						// do nothing
							...
							$insertArr['session_id'] = session_id();
							$this->Currentpatron->save($insertArr);
                                                        $this->log("users/plogin: [libid=".$existingLibraries['0']['Library']['id'].", patronid=".$patronId.", session_id=".$insertArr['session_id'].", CNT=".count($currentPatron).", last inserted ID=".$this->Currentpatron->getLastInsertId()."]", "currentpatrons");
						}
						$date = time();
						$values = array(0 => $date, 1 => session_id());
						Cache::write("login_".$existingLibraries['0']['Library']['library_territory']."_".$existingLibraries['0']['Library']['id']."_".$patronId, $values);
				   ...
				   $this->redirect(array('controller' => 'users', 'action' => 'ilhdlogin'));
					...
					if($resultAnalysis[0] == "fail"){
						...
						// do nothing
							...
							$insertArr['session_id'] = session_id();
							$this->Currentpatron->save($insertArr);
                                                        $this->log("users/ilhdlogin: [libid=".$existingLibraries['0']['Library']['id'].", patronid=".$patronId.", session_id=".$insertArr['session_id'].", CNT=".count($currentPatron).", last inserted ID=".$this->Currentpatron->getLastInsertId()."]", "currentpatrons");
						}
						$date = time();
						$values = array(0 => $date, 1 => session_id());
						Cache::write("login_".$existingLibraries['0']['Library']['library_territory']."_".$existingLibraries['0']['Library']['id']."_".$patronId, $values);
   ...
   */
				...
				if(count($existingLibraries) == 0){
					...
					if($resultAnalysis[0] == "fail"){
						...
						// do nothing
							...
							$insertArr['session_id'] = session_id();
							$this->Currentpatron->save($insertArr);
                                                        $this->log("users/clogin: [libid=".$existingLibraries['0']['Library']['id'].", patronid=".$patronId.", session_id=".$insertArr['session_id'].", CNT=".count($currentPatron).", last inserted ID=".$this->Currentpatron->getLastInsertId()."]", "currentpatrons");
						}
						$date = time();
						$values = array(0 => $date, 1 => session_id());
						Cache::write("login_".$existingLibraries['0']['Library']['library_territory']."_".$existingLibraries['0']['Library']['id']."_".$patronId, $values);
            ...
            if($this->data){
                ...
                if($card == ''){
                    ...
                    if(count($existingLibraries) == 0){
                        ...
                        $authData = array();
                        $authData['multilogin'] = 1;
                        $authData['patronId'] = $patronId;
                        ...
                        if($authMethod == 'innovative_var_wo_pin' || $authMethod == 'innovative_wo_pin' || $authMethod == 'innovative_var_https_name' || $authMethod == 'innovative_var_https_wo_pin' || $authMethod == 'innovative_var_name') {
                                ...
                                $data['library_authentication_url'] = $existingLibraries['0']['MultiAuthentication']['library_authentication_url'];                                        
                        } elseif($authMethod == 'mdlogin_reference' || $authMethod == 'mndlogin_reference') {
                            $login_res = $this->Card->find('first',array('conditions' => array('Card.card_number' => $card , 'Card.pin' => $pin , 'Card.library_id' => $existingLibraries['0']['Library']['id'] ) , 'fields' => array('id')));
                            if(isset($login_res['Card']['id'])) {
                                $this->authSuccessProcess($existingLibraries,$authData);
                            } else {
                                $this->Session->setFlash('Invalid Credentials');
                                $this->redirect(array('controller' => 'users', 'action' => 'multilogin'));
                            }
                        ...
                        if($resultAnalysis[0] == "fail"){
                            ...
                            if($authMethod == 'sip2_var' || $authMethod == 'sip2_var_wo_pin' || $authMethod == 'sip2' || $authMethod == 'sip2_wo_pin'){
                                $authData['branch'] = 1;
	...
	function admin_addmultipleusers($noOfUsers){
        ...
        function authSuccessProcess($existingLibraries,$authData){
            //writing to memcache and writing to both the memcached servers
            $currentPatron = $this->Currentpatron->find('all', array('conditions' => array('libid' => $existingLibraries['0']['Library']['id'], 'patronid' => $patronId)));
            ...
            if(!empty($authData['multilogin'])) {
                ...
                $authMethod = $existingLibraries['0']['Library']['library_authentication_method'];
                $logoutUrl = $existingLibraries['0']['Library']['library_logout_url'];
            }
            if(!empty($authData['branch'])) {
                $branch_name  = base64_decode($authData['result']['Posts']['branch_name']);
                $branch_name  = ($branch_name=='NA')?'':$branch_name;
            }
            if(count($currentPatron) > 0){
                if(!empty($authData['branch'])) {
                    $this->Currentpatron->id = $currentPatron[0]['Currentpatron']['id'];
                    $this->Currentpatron->saveField('branch_name',$branch_name, false);
                    $this->log("users/".$authMethod."(update): [id=".$this->Currentpatron->id.", CNT=".count($currentPatron)."]", "currentpatrons");
                }
            } else {
                $insertArr['libid'] = $existingLibraries['0']['Library']['id'];
                $insertArr['patronid'] = $patronId;
                ...
                $this->Currentpatron->save($insertArr);
                $this->log("users/".$authMethod.": [libid=".$existingLibraries['0']['Library']['id'].", patronid=".$patronId.", session_id=".$insertArr['session_id'].", CNT=".count($currentPatron)."]", "currentpatrons");
            }
            $date = time();
            $values = array(0 => $date, 1 => session_id());
            Cache::write("login_".$existingLibraries['0']['Library']['library_territory']."_".$existingLibraries['0']['Library']['id']."_".$patronId, $values);
            ...
            $this->Session->write("patron", $patronId);
            $this->Session->write("territory", $existingLibraries['0']['Library']['library_territory']);
            $this->Session->write($authMethod,$authMethod);
            if($logoutUrl != '' && ($this->Session->read('referral') != '' || $this->Session->read("subdomain") != '')){
                $this->Session->write("referral",$logoutUrl);
            }
            if(!$this->Session->read('Config.language') && $this->Session->read('Config.language') == ''){
                $this->Session->write('Config.language', $existingLibraries['0']['Library']['library_language']);
            }
