...
class salesfore_reports {
	...
	public function getLibraryIds($labels){
		$sql = "SELECT libraries.id, libraries.library_name, libraries.customer_id, libraries.library_streaming_hours FROM libraries WHERE libraries.library_status = 'active' AND libraries.library_type = 2 AND libraries.customer_id != 0";
		...
		foreach($result as $row){
			$libids[$row['id']] = array(
				...
				'contract_end_date' => '',
				'value_of_contract' => '0',
				'streaming_hours_per_day' => $row['library_streaming_hours'],
				$labels[0] => '0',
				$labels[1] => '0',
				$labels[2] => '0',
				$labels[3] => '0',
	...
	public function getContractInfo($final){
		foreach ($final as $library_id => $value) {
			$sql = "SELECT
						clsp.library_id 'id',
						clsp.library_contract_start_date 'contract_start_date',
						clsp.library_contract_end_date 'contract_end_date',
						lps.purchased_amount 'value_of_contract'
					FROM
						contract_library_streaming_purchases clsp
					JOIN library_purchases_streaming lps ON lps.id = clsp.id_library_purchases_streaming
					WHERE
						clsp.library_id = $library_id
					ORDER BY
						clsp.id DESC
					LIMIT 1";
			$stmt = $this->conn->prepare($sql);
