...
EOD;
	...
	public function createReport($file_name, $labels, $final) {
		...
		$header = 'customer_id,library_name,cte,contract_start_date,contract_end_date,value_of_contract,renewal_start_date,renewal_end_date,value_of_renewal,tokens_per_week,' . $labels[0] . ',' . $labels[1] . ',' . $labels[2] . ',' . $labels[3] . ',' . $labels[4] . "\n";
...
function sendMail($file_path, $file_name) {
	...
	$subject ='Freading.com: Library download report (' . date('ymd') . ')';
	$message =
			"Greetings,<br/><br/>
			Attached is a report that contains the total number of book downloads each library had during their contract period. This report also contains the total book downloads for each of the previous four months and each of the previous four weeks.<br/><br/>
			Thanks,<br/>
			The Freading Team<br/><br/>";
