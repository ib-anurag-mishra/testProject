...
if (($currentDate == $weekFirstDay) || ($currentDate == $monthFirstDate))
{
    ...
    foreach ($lib_types as $lib_type)
    {
        ...
        foreach ($countrys as $country => $currency)
        {
            ...
            {
                echo "\n Current date : $currentDate is week first day";
                ...
                $report_name = $reports_dir . "/PM43_W_" . $showStartDate . "_" . $showEndDate . "_" . $lib_type . "_" . $count . "_" . $country . "_SFV.txt";

                $sql = "SELECT COUNT(*) as ReportCount, id FROM sony_reports WHERE report_name = '$report_name'";
                ...
                if ($result3)
                {
                    ...
                    $sql = "SELECT lp.library_id,clp.library_contract_start_date,clp.library_contract_end_date,clp.library_unlimited,l.library_territory "
                            ...
                            . "AND ( (clp.library_contract_start_date <= '" . $condStartDate . "' AND clp.library_contract_end_date >= '" . $condEndDate . "')  "
                            . "OR (clp.library_contract_start_date <= '" . $condStartDate . "' AND clp.library_contract_end_date BETWEEN '" . $condStartDate . "' "
                        ...
                        if ($countno > 0)
                        {
                            while ($row = mysql_fetch_assoc($result))
                            {
                                ...
                                if ($row['library_contract_start_date'] <= $condStartDate)
                                {
                                    ...
                                    {
                                        $query = "SELECT 1 AS TrkCount, videodownloads.ISRC AS TrkID, videodownloads.artist,  videodownloads.track_title, "
                                                . "videodownloads.ProductID AS productcode,currentpatrons.id,videodownloads.library_id,videodownloads.created "
                                                . "FROM videodownloads "
                                                . "left join currentpatrons on currentpatrons.libid = videodownloads.library_id AND currentpatrons.patronid = videodownloads.patron_id "
                                                . "LEFT JOIN video on video.ProdID=videodownloads.ProdID AND video.provider_type=videodownloads.provider_type "
                                                . "WHERE videodownloads.provider_type='sony' "
                                                . "and videodownloads.created between '" . $condStartDate . "' and '" . $condEndDate . "' "
                                                . "and library_id = " . $library_id . " "
                                                . "group by videodownloads.id";
                                    }
                                    else
                                    {
                                        $query = "SELECT 1 AS TrkCount, videodownloads.ISRC AS TrkID, videodownloads.artist,  videodownloads.track_title, "
                                                ...
                                                . "LEFT JOIN video on video.ProdID=videodownloads.ProdID AND video.provider_type=videodownloads.provider_type "
                                                . "WHERE videodownloads.provider_type='sony' "
                                                . "and videodownloads.created between '" . $condStartDate . "' and '" . $row['library_contract_end_date'] . " 23:59:59' "
                                                . "and library_id = " . $library_id . " "
                                                . "group by videodownloads.id";
                                    }
                                }
                                else
                                {
                                    if ($row['library_contract_end_date'] >= $condEndDate)
                                    {
                                        $query = "SELECT 1 AS TrkCount, videodownloads.ISRC AS TrkID, videodownloads.artist,  videodownloads.track_title, "
                                                ...
                                                . "LEFT JOIN video on video.ProdID=videodownloads.ProdID AND video.provider_type=videodownloads.provider_type "
                                                . "WHERE videodownloads.provider_type='sony' "
                                                . "and videodownloads.created between '" . $row['library_contract_start_date'] . " 00:00:00' and '" . $condEndDate . "' "
                                                . "and library_id = " . $library_id . " "
                                                . "group by videodownloads.id";
                                    }
                                    else
                                    {
                                        $query = "SELECT 1 AS TrkCount, videodownloads.ISRC AS TrkID, videodownloads.artist,  videodownloads.track_title, "
                                                ...
                                                . "LEFT JOIN video on video.ProdID=videodownloads.ProdID AND video.provider_type=videodownloads.provider_type "
                                                . "WHERE videodownloads.provider_type='sony' "
                                                . "and videodownloads.created between '" . $row['library_contract_start_date'] . " 00:00:00' and '" . $row['library_contract_end_date'] . " 23:59:59' "
                                                . "and library_id = " . $library_id . " "
                                                . "group by videodownloads.id";
                                    }
                                }
                                    ...
                                    while ($datarow = mysql_fetch_assoc($dataresult))
                                    {
                                        ...
                                        if (!empty($data) || !empty($videodata))
                                        {
                                            ...
                                            if ($result6)
                                            {
                                                ...
                                                if (sendReportFilesftp($report_name, $report_name, $logFileWrite, "weekly"))
                                                {
                    ...
                    {
                        sendalert("At Line  : Query failed: " . $sql);
                        die("At Line  : Query failed: " . $sql . " Error: " . mysql_error());
                    }
                }
                else
                {
                            ...
                            while ($row = mysql_fetch_assoc($result))
                            {
                                ...
                                if ($row['library_contract_start_date'] <= $condStartDate)
                                {
                                    if ($row['library_contract_end_date'] >= $condEndDate)
                                    {
                                        $query = "SELECT 1 AS TrkCount, videodownloads.ISRC AS TrkID, videodownloads.artist,  videodownloads.track_title, "
                                                ...
                                                . "FROM videodownloads "
                                                . "left join currentpatrons on currentpatrons.libid = videodownloads.library_id AND currentpatrons.patronid = videodownloads.patron_id "
                                                . "LEFT JOIN video on video.ProdID=videodownloads.ProdID "
                                                . "WHERE videodownloads.provider_type='sony' "
                                                . "and videodownloads.created between '" . $condStartDate . "' and '" . $condEndDate . "' and library_id = " . $library_id . " "
                                                . "group by videodownloads.id";
                                    }
                                    else
                                    {
                                ...
                                else
                                {
                                    if ($row['library_contract_end_date'] >= $condEndDate)
                                    {
                                        $query = "SELECT 1 AS TrkCount, videodownloads.ISRC AS TrkID, videodownloads.artist,  videodownloads.track_title, "
                                                ...
                                                . "FROM videodownloads "
                                                . "left join currentpatrons on currentpatrons.libid = videodownloads.library_id AND currentpatrons.patronid = videodownloads.patron_id "
                                                . "LEFT JOIN video on video.ProdID=videodownloads.ProdID WHERE videodownloads.provider_type='sony' "
                                                . "and videodownloads.created between '" . $row['library_contract_start_date'] . " 00:00:00' and '" . $condEndDate . "' "
                                                . "and library_id = " . $library_id . " "
                                                . "group by videodownloads.id";
                                    }
                                ...
                                if ($dataresult)
                                {
                                    ...
                                    if (!empty($data) || !empty($videodata))
                                    {
                                        ...
                                        {
                                            // FOR SENDING REPORT TO SONY SERVER USING SFTP
                                            if (sendReportFilesftp($report_name, $report_name, $logFileWrite, "monthly"))
                                            {
                                                // FOR SENDING REPORT TO SONY SERVER USING FTP
                                                $sql = "UPDATE sony_reports SET is_uploaded = 'yes', modified = now() WHERE id = " . mysql_insert_id();
                                                $result7 = mysql_query($sql);
                                ...
                                {
                                    sendalert("At Line  : Query failed: " . $query);
                                    die("At Line  : Query failed: " . $query . " Error: " . mysql_error());
                                }
                            }
                        }
                        else
            ...
            {
                echo "\n$currentDate is not either the week first day or the month first day so the report didn't get generated.\n";
            }
        }
    }
}
