...
function sendMail($file_path, $file_name) {
	...
	$subject ='Freegalmovies.com: Library download report (' . date('ymd') . ')';
	$message =
			"Greetings,<br/><br/>
			Attached is a report that contains the total number of movie streams each library had during their contract period. This report also contains the total movie streams for each of the previous four months and each of the previous four weeks.<br/><br/>
			Thanks,<br/>
			The Freegal Movies Team<br/><br/>";
...
function freegalMoviesStreams() {
	//$SalesforceReports = new salesfore_reports('localhost;port=3306', 'fmovies', 'root', '');
	$SalesforceReports = new salesfore_reports('192.168.100.114', 'fmovies', 'freegal_prod', '}e47^B1EO9hD');
