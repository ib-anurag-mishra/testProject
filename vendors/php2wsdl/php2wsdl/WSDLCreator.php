...
class WSDLCreator {
  ...
  /**
    ...
    $this->WSDLXML->setAttribute("targetNamespace", $url);
		//$this->WSDLXML->setAttribute("xmlns:typens", "urn:".$name);
    $this->WSDLXML->setAttribute("xmlns:typens", $url);
  ...
  /**
	 ...
	 * @param string $file
	 */
	public function setTnsURL ($url) {$this->tnsURL = $url; }
	...
	private function addComplexTypes($type) {
      ...
      $xsdComplexTypeSequence = new XMLCreator("xsd:sequence");
            ...
            $element = new XMLCreator("xsd:element");
						$element->setAttribute("name", $complexType);
            $element->setAttribute("minOccurs", "0");
            $element->setAttribute("maxOccurs", "unbounded");
            $element->setAttribute("nillable", "true");
            $element->setAttribute("type","tns:".$complexType);
						/*if(isset($this->xsd[strtolower($complexType)])) {
							...
							$arrayType =  "tns:ArrayOf".$complexType; //"urn:".$complexType."[]";
						}*/
      ...
      $xsdComplexTypeSequence->addChild($element);
      $xsdComplexType->addChild($xsdComplexTypeSequence);
		...
		if (is_array($params)) {
			foreach ($params as $pname=>$param) {
				...
				if (isset($this->xsd[strtolower($type)])) {
					...
					$type = "tns:ArrayOf".trim(substr($type,0,-2));
				} else {
      ...
      $type = isset($returnType) ? $returnType:"anyType";
			if (isset($this->xsd[strtolower($type)])) {
				...
				if (isset($this->typeTypens[$type])) {
					...
					$this->addComplexTypes($type,$name);
          $type = "tns:ArrayOf".trim(substr($type,0,-2));
        } else {
					$this->addtypens($type);
					$typens = isset($this->typensDefined[$type]) ? "tns" : $this->typeTypens[$type];
					$type = $typens.":".$type;
				}
			}
      $part->setAttribute("type", $type);
    ...
    if(!empty($this->tnsURL)){
      $this->WSDLXML->setAttribute("xmlns:tns", $this->tnsURL);
    }
		// add types
      ...
      $xsdSchema->setAttribute("targetNamespace", $this->tempURL);
			...
			foreach ($this->typensDefined as $typensDefined) {
				...
				$all = new XMLCreator("xsd:sequence");
            ...
            if(substr($varType, -2) == "[]"){
              $varType = "ArrayOf".substr($varType,0,-2);
            }
            $varType = isset($this->xsd[$varType]) ? "xsd:".$this->xsd[strtolower($varType)] : "tns:".$varType;
