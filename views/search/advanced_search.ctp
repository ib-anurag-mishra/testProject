...
?>
<link
	type="text/css" rel="stylesheet" href="/css/advanced_search.css">
...
<div id="leftCol">
	<div id="leftColWrapper">
		<form method="get" id="searchQueryForm">
			<h1>
				<label for="search_query">Search music on freegalmusic.com</label>
			</h1>
			<input type="text" id="search_query" value="<?php echo $keyword ?>"
				class="query" name="q"> <input type="hidden" id="search_type"
				value="<?php echo ( isset( $type ) && !empty( $type ) ) ? $type : 'all' ?>"
				name="type"> <input type="submit" value="search">
			<ul class="clearit" id="searchfilter">
				<li class=" current	first "><?php
				if( $type != 'all' ) {
					?> <a
					href="/search/advanced_search?q=<?php echo $keyword; ?>&type=all">All
						Music</a> <?php
				} else {
					?> <a href="javascript:void(0)" style="color: #000">All Music</a> <?php
				}
				?>
				</li>
				<li><?php
				if( $type != 'album' ) {
					?> <a
					href="/search/advanced_search?q=<?php echo $keyword; ?>&type=album">Albums</a>
					<?php
				} else {
					?> <a href="javascript:void(0)" style="color: #000">Albums</a> <?php
				}
				?>
				</li>
				<li><?php
				if( $type != 'artist' ) {
					?> <a
					href="/search/advanced_search?q=<?php echo $keyword; ?>&type=artist">Artists</a>
					<?php
				} else {
					?> <a href="javascript:void(0)" style="color: #000">Artists</a> <?php
				}
				?>
				</li>
				<li><?php
				if( $type != 'composer' ) {
					?> <a
					href="/search/advanced_search?q=<?php echo $keyword; ?>&type=composer">Composers</a>
					<?php
				} else {
					?> <a href="javascript:void(0)" style="color: #000">Composers</a> <?php
				}
				?>
				</li>
				<li><?php
				if( $type != 'genre' ) {
					?> <a
					href="/search/advanced_search?q=<?php echo $keyword; ?>&type=genre">Genres</a>
					<?php
				} else {
					?> <a href="javascript:void(0)" style="color: #000">Genres</a> <?php
				}
				?>
				</li>
				<li><?php
				if( $type != 'label' ) {
					?> <a
					href="/search/advanced_search?q=<?php echo $keyword; ?>&type=label">Label</a>
					<?php
				} else {
					?> <a href="javascript:void(0)" style="color: #000">Label</a> <?php
				}
				?>
				</li>
				<li id="list_last"><?php
				if( $type != 'song' ) {
					?> <a
					href="/search/advanced_search?q=<?php echo $keyword; ?>&type=song">Songs</a>
					<?php
				} else {
					?> <a href="javascript:void(0)" style="color: #000">Songs</a> <?php
				}
				?>
				</li>
			</ul>
		...
		<div id="ComposersWrapper">
			...
			<ul>
				...
				foreach( $composers as $composer ) {
					...
					?>
				<li><span class="left_text"><a
						href="/search/advanced_search?q=<?php echo $tilte;?>&type=composer"
						title='<?php echo $composer->Composer?>'><?php echo str_replace( '"', '', $composer_name );?>
					</a> </span><span class="right_text">(<?php echo $composer->numFound; ?>)
				</span></li>
				<?php
				}
				?>
			</ul>
			<span class="more_link"><a
				href="/search/advanced_search?q=<?php echo $keyword; ?>&type=composer">See
					more Composers</a> </span>
			<?php
			} else {
				?>
			<ul>
	...
	<div id="ArtistWrapper">
		...
		<ul>
			...
			foreach( $artists as $artist ) {
				...
				$link 			  = $html->link( str_replace( '"', '', truncate_text( $artist->ArtistText, 30, $this ) ), array( 'controller' => 'artists', 'action' => 'album', str_replace( '/', '@', base64_encode( $artist->ArtistText ) ) ) );
				?>
			<li><span class="left_text"><?php echo $link; ?> </span><span
				class="right_text">(<?php echo $artist->numFound; ?>)
			</span></li>
			<?php
			}
			?>
		...
		<span class="more_link"><a
			href="/search/advanced_search?q=<?php echo $keyword; ?>&type=artist">See
				more Artists</a> </span>
		<?php
		} else {
			?>
	...
	<div id="LabelWrapper">
		...
		<ul>
			...
			foreach($labels as $label) {
				...
				?>
			<li><span class="left_text"><a
					href="/search/advanced_search?q=<?php echo $tilte;?>&type=label"'<?php echo $name; ?>'><?php echo (($name!="false")?$label_name_text:""); ?>
				</a> </span><span class="right_text">(<?php echo $count; ?>)
			</span></li>
			<?php
			}
			?>
		...
		<span class="more_link"><a
			href="/search/advanced_search?q=<?php echo $keyword; ?>&type=label">See
				more Labels</a> </span>
		<?php
		} else {
			?>
...
<div>
	<div class="links" id="genreArtist" style="width: 192px;">
		<a
			href="<?php echo "/search/advanced_search/" . $currentPage . "/" . $facetPage . "/?q=" . $keyword . "&type=" . $type . "&sort=artist&sortOrder=" . ( ( $sort == 'artist' ) ? $reverseSortOrder : 'asc' );?>">Artist
			<?php
			if( $sort == 'artist' ) {
	...
	<div class="links" id="genreComposer" style="width: 180px;">
		<a
			href="<?php echo "/search/advanced_search/" . $currentPage . "/" . $facetPage . "/?q=" . $keyword . "&type=" . $type . "&sort=composer&sortOrder=" . ( ( $sort == 'composer' ) ? $reverseSortOrder : 'asc' );?>">Composer
			<?php
			if( $sort == 'composer' ) {
	...
	<div class="links" id="genreAlbum" style="width: 192px;">
		<a
			href="<?php echo "/search/advanced_search/" . $currentPage . "/" . $facetPage . "/?q=" . $keyword . "&type=" . $type . "&sort=album&sortOrder=" . ( ( $sort == 'album' ) ? $reverseSortOrder : 'asc' );?>">Album
			<?php
			if( $sort == 'album' ) {
	...
	<div class="links" id="genreTrack" style="width: 215px;">
		<a
			href="<?php echo "/search/advanced_search/" . $currentPage . "/" . $facetPage . "/?q=" . $keyword ."&type=" . $type . "&sort=song&sortOrder=" . ( ( $sort == 'song' ) ? $reverseSortOrder : 'asc' );?>">Track
			<?php
			if( $sort == 'song' ) {
			...
			<tbody>
				...
				<tr <?php echo $class; ?> style="margin-left: 0px;">
					<td width="187" valign="top" style="padding-left: 5px;">
						<p>
							<span
								title="<?php echo str_replace( '"', '', $this->getTextEncode( $psong->ArtistText ) );?>"><?php echo $html->link( str_replace( '"', '', truncate_text( $psong->ArtistText, 30, $this ) ), array( 'controller' => 'artists', 'action' => 'album', str_replace( '/', '@', base64_encode( $psong->ArtistText ) ) ) );?>
							</span>
						</p>
					</td>
					<td width="170" valign="top" style="padding-left: 10px;">
						<p>
							<span
								title="<?php echo str_replace( '"', '', $this->getTextEncode( $psong->Composer ) );?>"><?php echo truncate_text( str_replace( '"', '', $psong->Composer ), 30, $this ); ?>
							</span>
						</p>
					</td>
					<td width="182" valign="top" style="padding-left: 10px;">
						<p>
							<span
								title="<?php echo str_replace( '"', '', $this->getTextEncode( $psong->Title ) );?>"><a
								href="/artists/view/<?php echo str_replace( '/', '@', base64_encode( $psong->ArtistText ) );?>/<?php echo $psong->ReferenceID;	?>/<?php echo base64_encode( $psong->provider_type );?>"><?php echo str_replace( '"', '', truncate_text( $psong->Title, 30, $this ) );?>
							</a> </span>
						</p>
					</td>
					<td valign="top" width="205" style="padding-left: 10px;">
						<p>
							<?php
             						$showSongTitle = truncate_text( $psong->SongTitle, strlen( $psong->SongTitle ), $this );?>
							<span
								title="<?php echo str_replace( '"', '', $showSongTitle );?>"><?php echo truncate_text( $psong->SongTitle, 28, $this );?>
								<?php
								if( $psong->Advisory == 'T' ) {
					...
					<td width="203" valign="top" align="center"
						...
						<form method="Post" id="form<?php echo $psong->ProdID; ?>"
							...
							<span class="beforeClick" id="song_<?php echo $psong->ProdID; ?>">
								...
								onclick='userDownloadAll(<?php echo $psong->ProdID; ?>);'><?php __('Download Now');?>
							</a>
						...
						<p>
							<span class="beforeClick"
								id="wishlist<?php echo $psong->ProdID; ?>"><a href='#'
								onclick='Javascript: addToWishlist("<?php echo $psong->ProdID; ?>","<?php echo $song->provider_type; ?>");'><?php __("Add To Wishlist");?>
							</a> </span><span
						...
						<p>
							...
							?> <span
						title='<?php __("Coming Soon");?> ( <?php echo date("F d Y", strtotime($sales_date)); ?> )'><?php __("Coming Soon");?>
					</span> <?php
						}
						?>
					</td>
