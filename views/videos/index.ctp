                          ...
                          ?>
												<form method="post" id="form<?= $topDownload["Video"]["ProdID"]; ?>" action="/videos/download">
                                                    ...
                                                    <span class="beforeClick" id="download_video_<?= $topDownload["Video"]["ProdID"]; ?>">
                                                        <![if !IE]>
                                                        <a class="no-ajaxy top-10-download-now-button" href="javascript:void(0);" title="<?= __('IMPORTANT:  Please note that once you press Download Now you have used up one of your downloads, regardless of whether you then press Cancel or not.'); ?>" onclick='ga("send", "event", "Top Videos", "Video Download", "<?php echo $trackingTopTitle; ?>", 1); return wishlistVideoDownloadOthersToken("<?= $topDownload['Video']['ProdID']; ?>", "0", "<?= $topDownload['Video']['CdnPath']; ?>", "<?= $topDownload['Video']['FullLength_SaveAsName']; ?>", "<?= $topDownload["Video"]["provider_type"]; ?>");'><?= __('Download Now'); ?></a>
                                                        <![endif]>
                                                        <!--[if IE]>
                                                        <a class="no-ajaxy top-10-download-now-button" href="javascript:void(0);" title="<?= __('IMPORTANT:  Please note that once you press Download Now you have used up one of your downloads, regardless of whether you then press Cancel or not.'); ?>" onclick="ga('send', 'event', 'Top Videos', 'Video Download', '<?php echo $trackingTopTitle; ?>', 1); wishlistVideoDownloadIEToken('<?= $topDownload['Video']['ProdID']; ?>', '0','<?= $topDownload['Video']['provider_type']; ?>', '<?= $topDownload['Video']['CdnPath']; ?>', '<?= $topDownload['Video']['FullLength_SaveAsName']; ?>');"><?= __('Download Now'); ?></a>
                                                        <![endif]-->
                                                    </span>
