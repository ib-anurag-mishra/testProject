...
$tcpdf->SetSubject('Streaming Report');
$tcpdf->SetKeywords('Streaming, Report');
...
// start - Library Remaining Stream
// add a page
$tcpdf->AddPage();
...
$tcpdf->SetFont('', 'B');
$tcpdf->Cell(250, 7, 'Library Remaining Stream', 0, 0, 'C', 0);
$tcpdf->Ln();
...
// Header
$w = array(10, 50, 190);
$DownloadCount_header = array('', 'Library Name', 'Number of Remaining Stream');
...
foreach ($libraries_downloads as $k => $row) {
    if ($k % 27 == 0 && $k != 0) {
        ...
        $tcpdf->SetLineWidth(0.3);
        $tcpdf->SetFont('', 'B');
        $tcpdf->Cell(250, 7, 'Libraries Remaining Stream', 0, 0, 'C', 0);
        $tcpdf->Ln();
...
// end - Library Remaining Stream  
//----------------------------------------------------------------------------------------------  
// start - Total Streamed during Reporting Period
...
if (empty($arr_all_library_downloads)) {
    ...
    // Header
    $w = array(250);
    $DownloadCount_header = array('Total Streamed');
    for ($i = 0; $i < count($DownloadCount_header); $i++)
        $tcpdf->Cell($w[$i], 7, $DownloadCount_header[$i], 1, 0, 'C', 1);
    $tcpdf->Ln();
    // Color and font restoration
    ...
    foreach ($arr_all_library_downloads_data as $k => $row) {
        if ($k % 27 == 0 && $k != 0) {
            ...
            // Header
            for ($i = 0; $i < count($DownloadCount_header); $i++)
                $tcpdf->Cell($w[$i], 7, 'hello', 1, 0, 'C', 1);
            $tcpdf->Ln();
        }
        // Color and font restoration
        $tcpdf->SetFillColor(224, 235, 255);
    ...
    // Header
    $w = array(10, 50, 190);
    $DownloadCount_header = array('', 'Library Name', 'Total Streamed');
    for ($i = 0; $i < count($DownloadCount_header); $i++)
        $tcpdf->Cell($w[$i], 7, $DownloadCount_header[$i], 1, 0, 'C', 1);
    $tcpdf->Ln();
    // Color and font restoration
...
// end - Total Streamed during Reporting Period 
//------------------------------------------------------------------------------------------------
// start - Total Number of Patrons who have Streamed during Reporting Period
...
if (empty($arr_all_patron_downloads)) {
    ...
    // Header
    $w = array(250);
    $DownloadCount_header = array('Total Number of Patrons who have streamed during Reporting Period');
    for ($i = 0; $i < count($DownloadCount_header); $i++)
        $tcpdf->Cell($w[$i], 7, $DownloadCount_header[$i], 1, 0, 'C', 1);
    $tcpdf->Ln();
    // Color and font restoration
...
//Column titles
$header = array('', 'Library Name', 'Patron ID', 'Artists Name', 'Track title', 'Stream');
//$video_header = array('','Library Name', 'Patron ID', 'Artists Name', 'Video title', 'Stream');
