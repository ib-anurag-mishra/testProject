...
EOD;
    ...
    function getStreamNowLabel($streamUrl, $songTitle, $artistText, $songLength, $songProdId, $providerType, $section = null, $analytics = null)
    {

        $label = __('Stream Now', true);
        $songTitle = base64_encode($songTitle);
        $artistText = base64_encode($artistText);
        if ($section && $analytics) {
             $str = <<<EOD
           <a onclick="loadSong('$streamUrl','$songTitle','$artistText',$songLength,$songProdId,'$providerType'); ga('send', 'event', '$section', 'Stream', '$analytics', 1)"  class="album-preview" href="javascript:void(0);" >$label</a>        
EOD;
        } else {
             $str = <<<EOD
           <a onclick="loadSong('$streamUrl','$songTitle','$artistText',$songLength,$songProdId,'$providerType');"  class="album-preview" href="javascript:void(0);" >$label</a>        
EOD;
        }
    ...
    function getNationalsongsStreamNowLabel($cdnPath, $sourceUrl,$songTitle, $artistText, $songLength, $songProdId, $providerType, $analytics)
    {
        ...
        $str = <<<EOD
       <button class="play-btn-icon" onclick="loadNationalTopSong('$cdnPath','$sourceUrl','$songTitle','$artistText','$songLength',$songProdId,'$providerType'); ga('send', 'event', 'Top Singles', 'Stream', '$analytics', 1)"></button>        
EOD;
        return $str;
    } 
...
EOD;
...

    {
        ...
        $str = <<<EOD
            <a onclick="javascript:loadAlbumSong('{$playList}'); ga('send', 'event', '$section', 'Stream', '$analytics', 1)"  class="album-preview" href="javascript:void(0);" >$stream_label</a>
EOD;
            } else {
        $str = <<<EOD
...
$str = <<<EOD
            <button onclick="javascript:loadAlbumSong('{$playList}'); ga('send', 'event', 'Featured Artist and Composers', 'Stream', '$analytics', 1)" class="stream-artist">$stream_label</button>
EOD;
            return $str;  
...
EOD;
            ...
            $stream_label = __('Stream Now', true);
            $str = <<<EOD
            <a onclick="javascript:loadAlbumData('$albumSongs',$AlbumID); ga('send', 'event', 'Artist View', 'Stream', 'Album', 1)" class="album-preview" href="javascript:void(0);" >$stream_label</a>
EOD;
            return $str;            
        }
