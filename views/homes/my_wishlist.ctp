         ...
         if(is_array($wishlistResults) && count($wishlistResults) > 0){ 
			...
			?>
				...
				<div class="row clearfix wishlistsong"  id="wishlistsong-<?php echo $wishlistResults[$i]['wishlists']['id']?>">
					...
					?><?php if('T' == $wishlistResults[$i]["Song"]['Advisory']) { ?> <span style="color: red;display: inline;font-size: 6px;"> (Explicit)</span> <?php } ?></div>
		...
		<div class="my-video-wishlist-shadow-container" style="display:none;">
			<div class="my-video-wishlist-scrollable">
				...
				<div class="row clearfix" id="wishlistvideo-<?php echo $wishlistResultsVideo['WishlistVideo']['id']?>">
					...
					?><?php if('T' == $wishlistResultsVideo["Video"]['Advisory']) { ?> <span style="color: red;display: inline;font-size: 6px;"> (Explicit)</span> <?php } ?>
                                        </div>
					<!--<a class="add-to-wishlist-button" href="#"></a>-->
                        ...
                        <?php
                            ...
                            <span class="beforeClick" id="download_video_<?php echo $wishlistResultsVideo['WishlistVideo']['ProdID']; ?>">
                                <?php if($wishlistResultsVideo['Country']['SalesDate'] <= date('Y-m-d')) { ?>
                                                    <![if !IE]>
                                                            <a title="IMPORTANT: Please note that once you press `Download Now` you have used up one of your downloads, regardless of whether you then press 'Cancel' or not." href='#' onclick='return wishlistVideoDownloadOthers("<?php echo $wishlistResultsVideo['WishlistVideo']['ProdID']; ?>","<?php echo $wishlistResultsVideo['WishlistVideo']['library_id']; ?>", "<?php echo urlencode($finalVideoUrlArr[0]);?>", "<?php echo urlencode($finalVideoUrlArr[1]);?>", "<?php echo urlencode($finalVideoUrlArr[2]);?>", "<?php echo $wishlistResultsVideo['WishlistVideo']["provider_type"]; ?>");'><?php __('Download');?></a>
                                                    <![endif]>
                                                    <!--[if IE]>
                                                            <a title="IMPORTANT: Please note that once you press `Download Now` you have used up one of your downloads, regardless of whether you then press 'Cancel' or not." onclick='return wishlistVideoDownloadIE("<?php echo $downloadResult['Download']['ProdID']; ?>","<?php echo $wishlistResultsVideo['WishlistVideo']['library_id']; ?>","<?php echo $wishlistResultsVideo['WishlistVideo']['patron_id']; ?>");' href='<?php echo $finalVideoUrl; ?>'><?php __('Download');?></a>
                                                    <![endif]-->
                                <?php } else { ?>
                                                <![if !IE]>
                                                <?php __('Coming Soon'); ?>
