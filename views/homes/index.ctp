				...
				<span class="top-100-download-now-button">
					<form method="Post" id="form<?= $nationalTopSong["Song"]["ProdID"]; ?>" action="/homes/userDownload" class="suggest_text1">
						...
						<span class="beforeClick" style="cursor: pointer;" id="wishlist_song_<?= $nationalTopSong["Song"]["ProdID"]; ?>">
							<![if !IE]> 
							<a class="download-icon" title="<?php __('IMPORTANT: Please note that once you press `Download Now` you have used up one of your downloads, regardless of whether you then press `Cancel` or not.'); ?>" onclick="ga('send', 'event', 'Top Singles', 'Download', '<?php echo $count; ?>-<?php echo h(addslashes($nationalTopSong['Song']['SongTitle'])); ?>', 1); return wishlistDownloadOthersHome('<?= $nationalTopSong['Song']['ProdID']; ?>', '0', '<?= $nationalTopSong['Full_Files']['CdnPath']; ?>', '<?= $nationalTopSong['Full_Files']['SaveAsName']; ?>', '<?= $nationalTopSong["Song"]["provider_type"]; ?>',1)"></a> 
							<![endif]> <!--[if IE]>
