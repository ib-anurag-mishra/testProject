	...
	<?php
		foreach($artists as $key => $artist):
				if($artist['Artist']['territory'] == $this->Session->read('territory')){
                    if($key == 0) {
                        echo $html->link(
                            $html->image(<?php echo $cdnPath;?>.substr($artist['Artist']['artist_image'], 4), array("alt" => $artist['Artist']['artist_name'], "title" => $artist['Artist']['artist_name'], "height" => "215", "width" => "942")),
    ...
    <div id="featured_artist">
            ...
            foreach($featuredArtists as $key => $featuredArtist):
				if($featuredArtist['Featuredartist']['territory'] == $this->Session->read('territory')){
					if($key == 0) {
						echo $html->link(
							$html->image(<?php echo $cdnPath;?>.substr($featuredArtist['Featuredartist']['artist_image'], 4), array("alt" => "Featured Arstist", "height" => "215", "width" => "300")),
    ...
    <div id="newly_added">
            ...
            foreach($newArtists as $key => $newArtist):
				if($newArtist['Newartist']['territory'] == $this->Session->read('territory')){			
					if($key == 0) {
						echo $html->link(
							$html->image(<?php echo $cdnPath;?>.substr($newArtist['Newartist']['artist_image'], 4), array("alt" => "Newly Added Artist", "height" => "215", "width" => "300")),
