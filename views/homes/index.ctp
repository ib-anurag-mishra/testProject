			...
			<ul class="clearfix">
				...
				if (is_array($nationalTopAlbums) && count($nationalTopAlbums) > 0) {
					foreach ($nationalTopAlbums as $key => $value) {
						...
						$trackingTitle = $this->getValidText($this->getTextEncode($title));
						?>
				<li>
					<div class="album-cover-container">
						...
						if ($this->Session->read('library_type') == 2 && !empty($value['albumSongs'][$value['Album']['ProdID']])) {
							echo $this->Queue->getAlbumStreamLabel($value['albumSongs'][$value['Album']['ProdID']], 4, $count . '-' . $this->getTextEncode($title));
							?>
						<a href="ga('send', 'event', 'Top Albums', 'Toggle Playlists', '<?php echo $count; ?>-<?php echo $this->getValidText($this->getTextEncode($title)); ?>')" class="playlist-menu-icon toggleable no-ajaxy" href="#" title="<?php __('Add to a Playlist or Create a New Playlist'); ?>"></a>
						<ul>
						...
						<a onclick="ga('send', 'event', 'Top Albums', 'Add to Wishlist', '<?php echo $count; ?>-<?php echo $this->getValidText($this->getTextEncode($title)); ?>')" class="wishlist-icon toggleable no-ajaxy" href="#" title="<?php __('Add to Wishlist'); ?>"></a>
						<?php
						}
						?>
					...
					<div class="album-info">
						<p class="title">
							<a onclick="ga('send', 'event', 'Top Albums', 'Title Click', '<?php echo $count; ?>-<?php echo $this->getValidText($this->getTextEncode($title)); ?>')" title="<?php echo $this->getValidText($this->getTextEncode($value['Album']['AlbumTitle'])); ?>" href="/artists/view/<?= base64_encode($value['Album']['ArtistText']); ?>/<?= $value['Album']['ProdID']; ?>/<?= base64_encode($value['Album']['provider_type']); ?>"><?php echo $this->getTextEncode($title); ?>
							</a>
						</p>
						<p class="artist">
							<a onlick="ga('send', 'event', 'Top Albums', 'Artist Click', '<?php echo $count; ?>-<?php echo $this->getValidText($this->getTextEncode($title)); ?>')" title="<?php echo $this->getValidText($this->getTextEncode($value['Album']['ArtistText'])); ?>" href="/artists/album/<?php echo str_replace('/', '@', base64_encode($value['Album']['ArtistText'])); ?>/<?= base64_encode($value['Genre']['Genre']) ?>"><?php echo $this->getTextEncode($ArtistText); ?>
							</a>
						</p>
					</div>
				</li>
...
<section class="top-songs">
	...
	<div class="top-singles-carousel-container">
		<div class="top-singles-carousel carousel">
			...
			if ($nationalTopSong['Song']['status'] != 'avail') { ?>
				<span class="top-100-download-now-button">
					<form method="Post" id="form<?= $nationalTopSong["Song"]["ProdID"]; ?>" action="/homes/userDownload" class="suggest_text1">
						...
						<span class="beforeClick" style="cursor: pointer;" id="wishlist_song_<?= $nationalTopSong["Song"]["ProdID"]; ?>">
							<![if !IE]> 
							<a class="download-icon" title="<?php __('IMPORTANT: Please note that once you press `Download Now` you have used up one of your downloads, regardless of whether you then press `Cancel` or not.'); ?>" onclick="return wishlistDownloadOthersHome('<?= $nationalTopSong['Song']['ProdID']; ?>', '0', '<?= $nationalTopSong['Full_Files']['CdnPath']; ?>', '<?= $nationalTopSong['Full_Files']['SaveAsName']; ?>', '<?= $nationalTopSong["Song"]["provider_type"]; ?>',1); ga('send', 'event', 'Top Singles', 'Download', '<?php echo $count; ?>-<?php echo $this->getValidText($this->getTextEncode($nationalTopSong['Song']['SongTitle'])); ?>', 1)"></a> 
							<![endif]> <!--[if IE]>
				...
				<a class="download-icon song-downloaded" href='/homes/my_history' title="<?php __('You have already downloaded this song. Get it from your recent downloads'); ?>"></a>
						<?php
			}
		} else { ?>
	...
	?>
						...
						<a onclick="ga('send', 'event', 'Top Singles', 'Add to Wishlist', '<?php echo $count; ?>-<?php echo $this->getValidText($this->getTextEncode($nationalTopSong['Song']['SongTitle'])); ?>')" class="wishlist-icon no-ajaxy" title="<?php __('Add to Wishlist'); ?>" href="#"></a>
						...
						<a onclick="ga('send', 'event', 'Top Singles', 'Toggle Playlists', '<?php echo $count; ?>-<?php echo $this->getValidText($this->getTextEncode($nationalTopSong['Song']['SongTitle'])); ?>')" class="playlist-menu-icon no-ajaxy" href="#" title="<?php __('Add to a Playlist or Create a New Playlist'); ?>"></a>
						<ul>
							<li><a href="#" class="create-new-playlist"><?php __('Create New Playlist'); ?>...</a></li>
						</ul>
						<?php                        
                         ...
                         if ($this->Session->read("patron")) {
							if ($this->Session->read('library_type') == 2 && $nationalTopSong['Country']['StreamingSalesDate'] <= date('Y-m-d') && $nationalTopSong['Country']['StreamingStatus'] == 1) {
								...
								echo $html->image('sample-icon.png', array("class" => "preview play-btn", "style" => "cursor:pointer;display:block;", "id" => "play_audio" . $count, "onClick" => 'playSample(this, "' . $count . '", ' . $nationalTopSong['Song']['ProdID'] . ', "' . base64_encode($nationalTopSong['Song']['provider_type']) . '", "' . $this->webroot . '"); ga("send", "event", "Top Singles, "Play Sample", "' . $count . '-' . $this->getValidText($this->getTextEncode($nationalTopSong['Song']['SongTitle'])) . '")'));
						...
						<div class="song-title">
							<a onclick="ga('send', 'event', 'Top Singles', 'Title Click', '<?php echo $count; ?>-<?php echo $this->getValidText($this->getTextEncode($nationalTopSong['Song']['SongTitle'])); ?>')" title="<?php echo $this->getValidText($this->getTextEncode($nationalTopSong['Song']['SongTitle'])); ?> " href="/artists/view/<?= base64_encode($nationalTopSong['Song']['ArtistText']); ?>/<?= $nationalTopSong['Song']['ReferenceID']; ?>/<?= base64_encode($nationalTopSong['Song']['provider_type']); ?>"><?php echo $this->getTextEncode($songTitle); ?>
							</a>
						</div>
						<div class="artist-name">
							<a onclick="ga('send', 'event', 'Top Singles', 'Artist Click', '<?php echo $count; ?>-<?php echo $this->getValidText($this->getTextEncode($nationalTopSong['Song']['SongTitle'])); ?>')"
...
<section class="featured-artists" id="featured-artists-section">
	...
	<div class="featured-artists-grid clearfix"
		...
		<div class="featured-grid-item">
			<a onclick="ga('send', 'event', 'Featured Artist and Composers', 'Artwork Click', 'R<?php echo $row; ?>C<?php echo $column; ?>-<?php echo $this->getValidText($this->getTextEncode($ArtistText)); ?>')" href="/artists/album/<?= base64_encode($v['Featuredartist']['artist_name']); ?>">
			...
			<div class="featured-grid-menu">
				...
				<div class="featured-artist-ctas">
					...
					?>
					<a onclick="ga('send', 'event', 'Featured Artist and Composers', 'More By', 'R<?php echo $row; ?>C<?php echo $column; ?>-<?php echo $this->getValidText($this->getTextEncode($ArtistText)); ?>')" title="<?php __('More by'); ?> <?php echo $this->getTextEncode($ArtistText); ?>" class="more-by-artist" href="/artists/album/<?= base64_encode($v['Featuredartist']['artist_name']); ?>">
						<?php echo $this->getTextEncode($ArtistText); ?>
					</a>
				</div>
			</div>
