                                            ...
                                            if ($this->Session->read('library_type') == 2 && $nationalTopDownload[$i]['Country']['StreamingSalesDate'] <= date('Y-m-d') && $nationalTopDownload[$i]['Country']['StreamingStatus'] == 1)
                                            {
                                                ...
                                                else 
                                                {
                                                    ...
                                                    if(!empty($filePath))
                                                     ...
                                                     echo $this->Queue->getStreamNowLabel($nationalTopDownload[$i]['streamUrl'],$this->getValidText($this->getTextEncode($song_title)),$this->getValidText($this->getTextEncode($nationalTopDownload[$i]['Song']['ArtistText'])),$nationalTopDownload[$i]['totalseconds'],$nationalTopDownload[$i]['Song']['ProdID'],$nationalTopDownload[$i]['Song']['provider_type']);
