...
<?php                                  if($value['Country']['SalesDate'] <= date('Y-m-d')) {
                                        echo $html->image('/news/top-100/preview-off.png', array("alt" => "Play Sample", "class" => "preview", "title" => "Play Sample", "style" => "cursor:pointer;display:block;", "id" => "play_audio".$key, "onClick" => 'playSample(this, "'.$key.'", '.$value['Song']['ProdID'].', "'.base64_encode($value['Song']['provider_type']).'", "'.$this->webroot.'");')); 
